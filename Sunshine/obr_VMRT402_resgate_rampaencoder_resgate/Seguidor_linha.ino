...
void segue_linha(int leitura_verdade) {
  ...
  //segundo sensor à direita acionado
      ...
      curvaDir(100);  // correcao de curso
  }
  ...
  //dois sensores a direita sobre a linha preta
    ...
    if (caso_cor == 0) {                        //não é acionado o sensor de cor
      ...
      frente(100);                          //avança até o final da linha preta
      delay(l_linha);
      ...
      while (linha < 99) {
          curvaDir(100);                 // procura linha com o 3º sensor de refletância
        linha = leitura_sensor();
      }
  ...
  //caso sensores extremo e intermediário da esquerda estejam acionados
    ...
    if (caso_cor == 0) {                        //não é acionado o sensor de cor
      ...
      while (linha > 12 || linha == 0) {
        curvaEsq(100);                 // procura linha com o 3º sensor de refletância
        linha = leitura_sensor();
      }
      frente(100);
  ...
  //delay(1000);
    ...
    if (caso_cor == 0) {                        //não é acionado o sensor de cor
      ...
      while (linha >12 || linha == 0) {
        curvaEsq(100);                 // procura linha com o 2º sensor de refletância
        linha = leitura_sensor();
      }
    }
  }
  ...
  //caso todos os sensores estejam acionados
    ...
    if (caso_cor == 0) {                        //não é acionado o sensor de cor
      ...
      while (linha > 99 || linha == 0) {
          curvaDir(100);                 // procura linha com o 2º sensor de refletância
        linha = leitura_sensor();
      }
    }
  }
...
void segue_rampa(int leitura_verdade) {
  ...
  //dois sensores a direita sobre a linha preta
    ...
    curvaDir(100);                             //vai para frente 100osicionar o sensor de cor sobre a fita verde
  }
